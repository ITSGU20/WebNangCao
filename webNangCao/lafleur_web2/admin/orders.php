...
<div class="filter-bar card" style="margin-bottom:1.5rem">
  <form method="GET" style="display:flex;gap:1rem;flex-wrap:wrap;align-items:center">
    ...
    <a href="<?= ADMIN_URL ?>/orders.php" class="btn btn-secondary">Xoá lọc</a>
  </form>
</div>
...
<div class="card">
  ...
  <?= admin_pagination($paged, ADMIN_URL.'/orders.php', array_filter(['search'=>$search,'status'=>$statusF,'from'=>$dateFrom,'to'=>$dateTo])+($sortAddr?['sort_addr'=>1]:[])) ?>
</div>

<!-- Order detail modal -->
<?php if ($viewOrder): ?>
<div class="modal-overlay active" onclick="if(event.target===this)location.href='<?= ADMIN_URL ?>/orders.php'">
  <div class="modal" style="max-width:700px">
    <div class="modal-header">
      <h3>Đơn hàng #<?= $viewOrder['id'] ?></h3>
      <a href="<?= ADMIN_URL ?>/orders.php" class="modal-close">✕</a>
    </div>
    ...
    <div class="modal-footer" style="flex-wrap:wrap;gap:.5rem">
      <a href="<?= ADMIN_URL ?>/orders.php" class="btn btn-outline">Đóng</a>
